...
export type TelaahanSigner = {
  ...
  pangkat?: string | null
  golongan?: string | null
  jabatanTampil?: string | null // Tambahan untuk jabatan khusus
} | null
...
export function buildTelaahanStafDocument(props: TelaahanStafPdfProps): React.ReactElement<DocumentProps> {
  ...
  /**
   * PENYESUAIAN PENANDATANGAN
   * 1. Gunakan signer?.jabatanTampil jika diisi, jika tidak gunakan signer?.jabatan
   */
  const dari = signer?.jabatanTampil || signer?.jabatan || 'Kepala Bagian Organisasi'
  ...
  /**
   * PENYESUAIAN DATA SIGNER
   * Mengambil data dari props signer (snapshot)
   */
  const signerNama = signer?.nama ?? 'NAMA PENANDATANGAN'
  const signerNip = signer?.nip ?? '-------------------'
  const signerPangkat = fmtPangkatGolongan(signer?.pangkat ?? null, signer?.golongan ?? null)
